...
import { isThemePackActive, THEME_PACKS } from '../../lib/themePacks'
...
export function SettingsView() {
  ...
  const backgroundMediaCache = useStore(state => state.backgroundMediaCache)
  const loadBackgroundMediaCache = useStore(state => state.loadBackgroundMediaCache)
  const removeBackgroundMedia = useStore(state => state.removeBackgroundMedia)
  const clearBackgroundMedia = useStore(state => state.clearBackgroundMedia)
  const runResourceCleanup = useStore(state => state.runResourceCleanup)
  ...
  // Spec « Gestion du cache » : l'inventaire des fonds vidéo n'est lu que
  // lorsque la section Stockage est ouverte (jamais au boot).
  useEffect(() => {
    if (openSection === 'storage') void loadBackgroundMediaCache()
  }, [openSection, loadBackgroundMediaCache])
  ...
  return <div className="h-full overflow-y-auto p-4">
    ...
    <div className="space-y-4">
      ...
      {hasCap('themes.packs') && <AccordionSection id="theme-packs" title="Packs de thèmes" subtitle="Presets complets : accent, densité, texte, animations" icon=<Palette size={13} /> open={openSection === 'theme-packs'} onToggle={() => toggleSection('theme-packs')}>
        <p className="mb-3 text-[11px] leading-relaxed text-white/35">Un pack applique d’un clic une combinaison de couleur d’accent, densité, taille de texte et animations. Les réglages restent modifiables individuellement après application.</p>
        <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">{THEME_PACKS.map(pack => {
          const active = isThemePackActive(pack, { accentColor, uiDensity, textSize, motionMode })
          return <button key={pack.id} type="button" onClick={() => { setAccentColor(pack.accentColor); setUiDensity(pack.uiDensity); setTextSize(pack.textSize); setMotionMode(pack.motionMode) }} className={`rounded-xl border p-3 text-left transition ${active ? 'border-gold/40 bg-gold/[0.05]' : 'border-white/[0.07] bg-white/[0.02] hover:border-gold/25'}`}>
            <span className="flex items-center gap-2"><span className="h-5 w-5 rounded-full border border-white/25" style={{ backgroundColor: pack.accentColor }} /><span className="text-xs font-semibold text-white/75">{pack.name}</span>{active && <span className="ml-auto rounded-full bg-gold/15 px-2 py-0.5 text-[9px] font-semibold text-gold">Actif</span>}</span>
            <span className="mt-1.5 block text-[10px] leading-relaxed text-white/40">{pack.description}</span>
            <span className="mt-1.5 block text-[9px] uppercase tracking-wider text-white/28">Accent {pack.accentColor} · {pack.uiDensity === 'compact' ? 'Compacte' : 'Confortable'} · {pack.textSize === 'small' ? 'Petit' : pack.textSize === 'large' ? 'Grand' : pack.textSize === 'very-large' ? 'Très grand' : 'Normal'} · {pack.motionMode === 'reduced' ? 'Animations réduites' : pack.motionMode === 'enabled' ? 'Animations actives' : 'Auto'}</span>
          </button>
        })}</div>
      </AccordionSection>}
