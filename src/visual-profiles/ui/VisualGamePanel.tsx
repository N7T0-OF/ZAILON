  ...
  return <div className="flex-1 overflow-y-auto p-4">
    <div className="mx-auto max-w-5xl space-y-4">
      ...
      <section className="rounded-2xl border border-white/[0.07] bg-white/[0.015] p-4">
        ...
        <div className="mt-3 flex flex-wrap gap-2"><button type="button" disabled={!selected || busy} onClick={() => void test()} className="flex items-center gap-2 rounded-lg bg-gold px-3 py-2 text-xs font-semibold text-[var(--zailon-accent-text)] disabled:opacity-30"><Play size={13} />Tester le profil</button><button type="button" disabled={busy} onClick={() => void visualBackend.restore(selected?.monitorId).then(result => setStatus(`${result.restored} écran(s) restauré(s).`)).catch(reason => setStatus(messageOf(reason)))} className="flex items-center gap-2 rounded-lg border border-white/[0.09] px-3 py-2 text-xs text-white/58"><RotateCcw size={13} />Restaurer maintenant</button></div>
      </section>
