...
| Fournisseur | Documentation vérifiée | Base / authentification | Pagination et limites | État ZAILON 1.3 |
|---|---|---|---|---|
...
| Nexus Mods | Fichier utilisateur « Nexus Mods API 3.0.0 », [API Acceptable Use Policy](https://help.nexusmods.com/article/114-api-acceptable-use-policy), [limites API](https://help.nexusmods.com/article/105-i-have-reached-a-daily-or-hourly-limit-api-requests-have-been-consumed-rate-limit-exceeded-what-does-this-mean) | Le document 3.0 fourni indique `ApiKeyAuthBearerJwtAuth`, mais ne fournit pas une base v3 exploitable pour tout le catalogue. Une application publique doit être enregistrée auprès de Nexus. Les clés personnelles ne conviennent qu’aux tests/usages personnels. | 20 000 requêtes par 24 h, puis 500 par heure selon la documentation Nexus consultée. Les quotas doivent être lus depuis les en-têtes de réponse. | Coffre d’identifiants du système, révocation, modèle `ExternalModReference`, niveaux de confiance et association Windows `nxm://` avec consentement : **implémentés**. Catalogue, résolution de fichiers et téléchargement automatiques : **bloqués** jusqu’à l’enregistrement officiel de ZAILON et la fourniture des paramètres d’application. ZAILON ne simule pas ces résultats. |
