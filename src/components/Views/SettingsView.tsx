...
export function SettingsView() {
  ...
  return <div className="h-full overflow-y-auto p-4">
    ...
    <div className="space-y-4">
      ...
      <section className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-3"><div className="mb-3 flex items-center gap-2 text-gold/70"><Database size={13} /><h2 className="text-[11px] font-mono uppercase tracking-widest">Library statistics</h2></div><div className="grid grid-cols-3 gap-2 text-center"><Stat label="Games" value={String(games.length)} /><Stat label="Mods" value={String(games.reduce((sum, game) => sum + game.profiles.reduce((total, profile) => total + profile.mods.length, 0), 0))} /><Stat label="Playtime" value={formatTime(totalPlaytime)} /></div></section>
