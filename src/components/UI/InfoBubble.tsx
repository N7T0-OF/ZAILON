 ...
 */
export function InfoBubble({ text, children }: { text: string; children?: ReactNode }) {
  ...
  return <span ref={root} className="relative inline-flex align-middle" onMouseEnter={() => schedule(true)} onMouseLeave={() => schedule(false)}>
    ...
    {open && <span id={id} role="tooltip" className="absolute bottom-full left-1/2 z-[300] mb-1.5 w-64 -translate-x-1/2 rounded-lg border border-white/[0.1] bg-[#141818] p-2.5 text-left text-[11px] leading-relaxed text-white/55 shadow-2xl">
      {text}
      {children}
    </span>}
  </span>
