...
import { OFFICIAL_ADDON_CATALOG, resolveAddonDownloadUrl, validateAddonManifest } from '../../src/lib/addons.ts'
...
test('le catalogue référence le SHA-256 et la taille réels des packages (spec §38)', () => {
  for (const id of officialIds) {
    ...
    assert.equal(entry.available, false, `${id} : available:false tant que la release GitHub n'est pas publiée (§49)`)
    assert.ok(entry.release, `${id} : release explicite prête`)
    assert.ok(resolveAddonDownloadUrl(entry), `${id} : URL résolue depuis release`)
  }
})
