    ...
    return <>
      <div className="relative flex h-full min-h-[480px] items-center justify-center overflow-hidden bg-[#0c0e0e] p-7 text-center">
        ...
        <div className="relative max-w-xl">
          ...
          <p className="mt-6 font-mono text-[7px] uppercase tracking-[0.28em] text-white/30">Universal Mod Launcher</p>
          <h1 className="mt-2 font-display text-5xl font-black uppercase leading-[0.88] text-white">Créez votre<br />bibliothèque</h1>
          <p className="mx-auto mt-4 max-w-md text-[10px] leading-relaxed text-white/38">Ajoutez un exécutable local ou détectez les bibliothèques installées sur cet appareil.</p>
          <div className="mt-6 flex flex-wrap justify-center gap-2">
            <button type="button" onClick={() => void addGameFromExecutable()} className="flex items-center gap-2 rounded-full bg-[#dbe8e5] px-5 py-2.5 text-[10px] font-bold uppercase tracking-[0.12em] text-[#101313] hover:bg-white"><FolderPlus size={12} /> Ajouter</button>
            <button type="button" onClick={() => setDiscoveryOpen(true)} className="flex items-center gap-2 rounded-full border border-white/[0.12] bg-black/22 px-5 py-2.5 text-[10px] uppercase tracking-[0.12em] text-white/58 backdrop-blur hover:bg-white/[0.07] hover:text-white"><Radar size={12} /> Détecter</button>
          </div>
        </div>
      </div>
    ...
    >
      ...
      <div className="relative flex h-full min-h-[520px] flex-col px-[clamp(1.25rem,4vw,4.5rem)] pb-4 pt-5">
        <header className="flex items-start justify-between gap-4">
          <div className="pt-1">
            <p className="font-mono text-[7px] uppercase tracking-[0.24em] text-white/30">Sélection actuelle</p>
            <p className="mt-1 text-[8px] text-white/22">{selectedGame.provider || 'Bibliothèque locale'}{selectedGame.version ? ` · v${selectedGame.version}` : ''}</p>
          </div>

          <div className="flex items-center gap-2">
            <div className="mr-1 hidden text-right sm:block">
              <p className="font-mono text-[7px] uppercase tracking-[0.15em] text-white/74">{isPlaying ? 'En jeu' : 'Prêt à jouer'}</p>
              <p className="mt-0.5 text-[7px] text-white/27">{isPlaying ? formatSeconds(sessionTime) : selectedGame.lastPlayed ? timeAgo(selectedGame.lastPlayed) : 'Jamais lancé'}</p>
            </div>
        ...
        <div className="mt-[clamp(2.2rem,8vh,6.5rem)] max-w-[min(690px,72vw)]">
          <p className="font-mono text-[7px] uppercase tracking-[0.25em] text-white/32">{selectedGame.favorite ? 'Jeu favori' : selectedGame.itemKind === 'software' ? 'Application locale' : 'Jeu sélectionné'}</p>
          {logo
          ...
          <p className="mt-5 text-[9px] text-white/38">Profil <span className="font-semibold text-white/70">{selectedProfile.name}</span><span className="mx-2 text-white/18">•</span>{activeMods} mod{activeMods !== 1 ? 's' : ''} actif{activeMods !== 1 ? 's' : ''}</p>
        ...
        <div className="mt-auto grid gap-2 pt-8 min-[800px]:grid-cols-[1.08fr_0.92fr_1.14fr]">
            ...
            <div className="flex h-[72px] items-end gap-3">
              ...
              <div className="flex min-w-0 flex-1 items-end justify-between gap-1.5">
                {activity.map((value, index) => <div key={index} className="flex min-w-0 flex-1 flex-col items-center gap-1" title={selectedGame.profiles[index]?.name || 'Aucun profil'}>
                  ...
                  <span className="max-w-full truncate font-mono text-[5px] uppercase text-white/18">{selectedGame.profiles[index]?.name.charAt(0) || '·'}</span>
                </div>)}
              </div>
            </div>
            <p className="mt-1 text-[8px] text-white/26">{profileActivity ? `${formatTime(profileActivity)} sur les profils` : 'Aucune activité enregistrée'}</p>
          </DashboardPanel>
          ...
          <DashboardPanel eyebrow="Vos statistiques" footer="Gérer les mods" onFooter={() => setView('mods')}>
            ...
            <p className="mt-1 truncate text-[8px] text-white/26">{selectedGame.lastPlayed ? `Dernière session ${timeAgo(selectedGame.lastPlayed)}` : 'Prêt pour une première session'}</p>
          </DashboardPanel>
          ...
          <DashboardPanel eyebrow="Bibliothèque récente" footer="Toute la bibliothèque" onFooter={() => setView('games')}>
            ...
            <p className="mt-1 truncate text-[8px] text-white/26">{visibleGames.length} élément{visibleGames.length !== 1 ? 's' : ''} dans ZAILON</p>
          </DashboardPanel>
        </div>
      </div>
    </section>
...
function DashboardPanel({ eyebrow, footer, onFooter, children }: { eyebrow: string; footer: string; onFooter: () => void; children: React.ReactNode }) {
  return <article className="min-w-0">
    <p className="mb-2 font-mono text-[6px] uppercase tracking-[0.22em] text-white/26">{eyebrow}</p>
    <div className="overflow-hidden rounded-xl border border-white/[0.055] bg-black/25 shadow-[0_14px_38px_rgba(0,0,0,0.18)] backdrop-blur-md">
      ...
      <button type="button" onClick={onFooter} className="w-full border-t border-white/[0.045] bg-black/10 py-2 font-mono text-[6px] uppercase tracking-[0.16em] text-white/18 hover:bg-white/[0.035] hover:text-white/46">{footer}</button>
    </div>
  </article>
}
...
function MiniStat({ icon: Icon, value, label }: { icon: typeof Boxes; value: string; label: string }) {
  return <div className="flex min-w-0 flex-col justify-center px-3 first:pl-0 last:pr-0">
    ...
    <p className="mt-1 truncate text-[7px] text-white/24">{label}</p>
  </div>
}
...
function QuickGame({ game, active, onSelect }: { game: Game; active: boolean; onSelect: () => void }) {
  ...
  return <button type="button" onClick={onSelect} title={game.name} className={`group/quick relative min-w-0 overflow-hidden rounded-lg border text-left ${active ? 'border-[#dbe8e5]/28' : 'border-white/[0.06] hover:border-white/20'}`}>
    ...
    <span className="absolute inset-x-1.5 bottom-1.5 truncate text-[7px] font-semibold text-white/78">{game.name}</span>
  </button>
}
