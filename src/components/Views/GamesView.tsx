import { FolderOpen, FolderPlus, Palette, Plus, Radar, RefreshCw, Search, Trash2 } from 'lucide-react'
  ...
  return (
    <div className="flex h-full">
      <aside className="flex w-40 flex-col border-r border-white/[0.05]">
        ...
        <button onClick={() => setSteamDialogOpen(true)} className="m-2 flex items-center justify-center gap-1 rounded border border-white/[0.08] py-1.5 text-[9px] text-white/45 hover:text-white/75"><Radar size={10} /> Detect games</button>
      </aside>
      <section className="flex min-w-0 flex-1 flex-col">
        ...
        {tab === 'mods' && <div className="flex-1 overflow-y-auto p-3"><p className="mb-2 rounded border border-amber-300/15 bg-amber-300/[0.04] px-2 py-1.5 text-[9px] text-amber-100/65">Priorité et conflits sont calculés à partir des fichiers réellement présents dans le dossier de mods. En déploiement direct, l’ordre de chargement final dépend encore du moteur du jeu.</p><div className="space-y-1">{filteredMods.length ? filteredMods.map((mod, index) => <ModCard key={mod.id} mod={mod} onToggle={() => void toggleMod(mod.id)} onDelete={() => void deleteMod(mod.id)} onMoveUp={index > 0 ? () => moveMod(mod.id, -1) : undefined} onMoveDown={index < filteredMods.length - 1 ? () => moveMod(mod.id, 1) : undefined} onNoteChange={note => setModNote(mod.id, note)} />) : <div className="flex h-32 flex-col items-center justify-center gap-2 text-[10px] text-white/35"><FolderPlus size={17} /><span>No mod found. Select a folder and scan it.</span></div>}</div></div>}
        {tab === 'profiles' && <div className="flex-1 space-y-3 overflow-y-auto p-3"><p className="text-[10px] text-white/45">Un profil conserve un état de mods et son ordre. L’appliquer renomme réellement les entrées activées/désactivées dans le dossier de mods.</p><div className="space-y-1.5">{selectedGame.profiles.map(profile => <div key={profile.id} className={`flex items-center gap-2 rounded border px-2 py-2 ${profile.id === selectedProfile.id ? 'border-gold/20 bg-gold/[0.04]' : 'border-white/[0.07] bg-white/[0.02]'}`}><input value={profile.name} onChange={event => renameProfile(profile.id, event.target.value)} className="min-w-0 flex-1 bg-transparent text-[10px] text-white/75 outline-none" /><span className="text-[9px] text-white/35">{profile.mods.filter(mod => mod.enabled).length} actif(s)</span><button onClick={() => removeProfile(profile.id)} disabled={selectedGame.profiles.length < 2} title="Supprimer le profil" className="rounded p-1 text-white/30 hover:text-red-300 disabled:opacity-20"><Trash2 size={11} /></button></div>)}</div><div className="flex gap-2"><input value={profileName} onChange={event => setProfileName(event.target.value)} onKeyDown={event => { if (event.key === 'Enter') { addProfile(profileName); setProfileName('') } }} placeholder="Nom du nouveau profil" className="flex-1 rounded border border-white/[0.08] bg-white/[0.04] px-2 py-1.5 text-[10px] text-white/70 outline-none focus:border-gold/30" /><button onClick={() => { addProfile(profileName); setProfileName('') }} className="rounded bg-gold px-2 text-[10px] font-semibold text-ink-400">Créer</button></div></div>}
        {tab === 'settings' && <div className="flex-1 space-y-3 p-3"><Field label="Game executable" value={selectedGame.execPath || ''} placeholder="Select the game executable" onChange={value => void setGamePath(selectedGame.id, value)} onBrowse={() => void browseExecutable()} /><Field label="Mods folder" value={selectedGame.modsPath || ''} placeholder="Select the mods folder" onChange={value => setModsPath(selectedGame.id, value)} onBrowse={() => void browseModsFolder()} /><button onClick={() => setResourcesOpen(true)} className="flex items-center gap-1.5 rounded border border-white/[0.1] bg-white/[0.03] px-2.5 py-1.5 text-[10px] text-white/65 hover:bg-white/[0.08]"><Palette size={11} /> Identité visuelle et ressources</button><p className="pt-1 text-[10px] font-mono text-white/35">Total playtime: {formatTime(selectedGame.totalPlaytime)}</p></div>}
      </section>
      {steamDialogOpen && <SteamDetectionDialog onClose={() => setSteamDialogOpen(false)} onImport={importDetectedGames} />}
      {resourcesOpen && <GameResourcesDialog game={selectedGame} onClose={() => setResourcesOpen(false)} onChange={resources => setGameResources(selectedGame.id, resources)} />}
    </div>
  )
}
