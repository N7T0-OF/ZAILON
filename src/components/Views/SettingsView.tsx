...
export function SettingsView() {
  ...
  const [openSection, setOpenSection] = useState<string | null>(() => {
    try {
      const stored = localStorage.getItem('zailon.settingsOpenSection')
  ...
  const toggleSection = (sectionId: string) => {
    ...
    setOpenSection(next)
    try {
      if (next) localStorage.setItem('zailon.settingsOpenSection', next)
      else localStorage.removeItem('zailon.settingsOpenSection')
  ...
  const revealSection = (label: string) => {
    ...
    if (!id) return
    setOpenSection(id)
    try { localStorage.setItem('zailon.settingsOpenSection', id) } catch { /* ignore */ }
  }
  ...
  return <div className="h-full overflow-y-auto p-4">
    ...
    <div className="space-y-4">
      ...
      <AccordionSection id="illustrations" title="Illustrations" subtitle="Images automatiques, sources, état" icon=<Palette size={13} /> open={openSection === 'illustrations'} onToggle={() => toggleSection('illustrations')}>
      ...
      <AccordionSection id="providers" title="Fournisseurs de mods" subtitle="Clés API, état des fournisseurs" icon=<KeyRound size={13} /> open={openSection === 'providers'} onToggle={() => toggleSection('providers')}>
      ...
      <AccordionSection id="nxm" title="Liens Nexus NXM" subtitle="Association nxm://" icon=<Link2 size={13} /> open={openSection === 'nxm'} onToggle={() => toggleSection('nxm')}>
